...
public class PooledConnection implements PooledConnectionMBean {
    ...
    public boolean shouldForceReconnect(String username, String password) {
        ...
        if (username == null) getAttributes().remove(PROP_USER);
        else getAttributes().put(PROP_USER, username);
        if (password == null) getAttributes().remove(PROP_PASSWORD);
        else getAttributes().put(PROP_PASSWORD, password);
