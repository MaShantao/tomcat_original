...
public class NioEndpoint extends AbstractJsseEndpoint<NioChannel> {
    ...
    protected boolean setSocketOptions(SocketChannel socket) {
        ...
        try {
            ...
            if (channel == null) {
                ...
                if (isSSLEnabled()) {
                    // 如果支持SSL，也就是支持HTTPS协议
                    channel = new SecureNioChannel(socket, bufhandler, selectorPool, this);
                } else {
                    // HTTP1.1
                    // 将SocketChannel转成NioChannel
    ...
    protected class Acceptor extends AbstractEndpoint.Acceptor {
        ...
        public void run() {
            ...
            // 如果服务器已经运行起来了，就一直监听客户端的请求，直到收到shutdown的命令
            while (running) {
                // 如果endpoint是paused的话，就循环。
                while (paused && running) {
                    state = AcceptorState.PAUSED;
                    try {
                        Thread.sleep(50); //暂停50微秒。
                    } catch (InterruptedException e) {
                        // Ignore
                    }
                }
                ...
                try {
                    ...
                    // 配置Socket
                    if (running && !paused) {
                        // 如果服务器正在运行，并且没有paused
                        // setSocketOptions会交给合适的处理器去处理socket
                        if (!setSocketOptions(socket)) {
                            // 如果socket处理成功的话，就关闭socket连接。
                            closeSocket(socket);
                        }
                    } else {
                        closeSocket(socket);
     ...
     * Poller class.
     */
    // Poller本质上是一个轮询selector的线程，不断轮询selector的事件
    public class Poller implements Runnable {
        ...
        public void register(final NioChannel socket) {
            ...
            PollerEvent r = eventCache.pop();
            // 拿到PollerEvent，将该事件注册到Poller的事件队列里面
            // 读事件
            ka.interestOps(SelectionKey.OP_READ); //this is what OP_REGISTER turns into.
            // 生成一个注册事件，并构造一个PollerEvent对象，
            if (r == null) r = new PollerEvent(socket, ka, OP_REGISTER);
            else r.reset(socket, ka, OP_REGISTER);
            // 将注册的Poller事件加到PollerEvent队列
            addEvent(r);
        }
        ...
        public void run() {
            ...
            while (true) {
                ...
                try {
                    if (!close) {
                        // 处理所有的PollerEvent，并执行PollerEvent的run方法。
                        hasEvents = events();
                        if (wakeupCounter.getAndSet(-1) > 0) {
                            ...
                            // 获得已准备的selector事件。
                            keyCount = selector.select(selectorTimeout); // 阻塞的select方法。
                        }
                        wakeupCounter.set(0);
                    }
                    if (close) {
